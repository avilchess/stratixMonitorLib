...
int main (int argc, char *argv[]) {
    ...
    auto counter1 = StratixMonitor->get_hardware_counters();
    std::cout << "Counter1: " << counter1.getCounters()[0] << std::endl;

    std::this_thread::sleep_for(std::chrono::milliseconds(2000));
    auto counter2 = StratixMonitor->get_hardware_counters();
    std::cout << "Counter2: " << counter2.getCounters()[0] << std::endl;

    std::this_thread::sleep_for(std::chrono::milliseconds(5000));
    auto counter3 = StratixMonitor->get_hardware_counters();
